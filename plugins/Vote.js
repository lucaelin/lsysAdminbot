...
module.exports = class Vote {
  constructor(bot, chat_id, question, values, cb, ...args) {
    this.bot = bot;
  ...
  async prepare() {
    ...
    this.votemsg = await this.tg.sendMessage(this.chat_id, `I am putting this to a vote! ${this.question}`, kbd);
    this.resultmsg = await this.tg.sendMessage(this.chat_id, 'There have been no votes, yet.');
